...
parser.add_argument("video_path", default="Chinatown_in_Lockdown.mp4", help="Path to input video file")
...
parser.add_argument("--video", action="store_true",
                    help="uses trained calibration from demo video")

parser.add_argument('--hide-result',action="store_true", help="Show output after processing each frame")
...
def main():
    ...
    while success:
        ...
        vid_msec = vidcap.get(cv2.CAP_PROP_POS_MSEC)

        success, truth_img = vidcap.read()
        # vidcap.set(cv2.CAP_PROP_POS_MSEC,(count*5000))    # added this line 
        print ('Read a new frame %d at %d: '% (count, vid_msec), success)
        img_id = "frame{0:08d}-{1:08d}".format(count, int(vid_msec))
        ...
        if not FLAGS.hide_result:
            if FLAGS.show_yolo:
                numpy_vertical = np.concatenate((truth_img, img), axis=0)
                cv2.imshow('SPACE for next image, any other key to exit', numpy_vertical)
            else:
                cv2.imshow('3D detections', img)
        cv2.imwrite('results/%s.jpg'%img_id, img)
